 ...
 * <br>
 * Yates, D., Islam, M.Z. (2020) FastForest: Increasing Random Forest Processing Speed While Maintaining Accuracy<br>
 * <br>
 * 
 * RandomTree.java modified by (C) 2019-2021 Darren Yates<br>
 * 
 * <!-- globalinfo-end -->
 * 
 ...
 * 
 */
public class RandomTreeFF extends AbstractClassifier implements OptionHandler,
...
//--------------------------------------------------------------------------------
    protected boolean useDDi = true;
    protected double propDDi = 1.0;
    protected boolean useHopping = true;
        
    protected int numRecords = 0;
    protected int overallRecords = 0;
...
//--------------------------------------------------------------------------------
     ...
     * in RandomForest
     *
     * @param computeImpurityDecreases true to compute and store impurity
     * decrease values for splitting attributes
     */
     ...
     * in RandomForest
     *
     * @return true to compute and store impurity decrease values for splitting
     * attributes
     */
...
//        int[] attIndicesWindow = new int[data.numAttributes() - 1];
        Integer[] attIndicesWindow = new Integer[data.numAttributes() - 1];
        int j = 0;
        ...
        if (data.classAttribute().isNumeric()) {
            trainVariance
                    = RandomTreeFF.singleVariance(classProbs[0], totalSumSquared, totalWeight)
                    / totalWeight;
    ...
    public String toString() {
        ...
        if (m_Tree == null) {
            return "RandomTreeFF: no model has been built yet.";
        } else {
            return "\nRandomTreeFF\n==========\n"
                    + m_Tree.toString(0)
                    + "\n"
    ...
    public String graph() throws Exception {
        ...
        if (m_Tree == null) {
            throw new Exception("RandomTreeFF: No model built yet.");
        }
        StringBuffer resultBuff = new StringBuffer();
        m_Tree.toGraph(resultBuff, 0, null);
        String result
                = "digraph RandomTreeFF {\n" + "edge [style=bold]\n" + resultBuff.toString()
                + "\n}\n";
        return result;
    }
    ...
    protected class Tree implements Serializable {
        ...
        /**
         * Class probabilities from the training data in the nominal case. Holds
         * the mean in the numeric case.
         */
        ...
        protected String toString(int level) {
            ...
            try {
                ...
                e.printStackTrace();
                return "RandomTreeFF: tree can't be printed";
            }
        }
        ...
        protected void backfitData(Instances data, double[] classProbs,
            ...
            if (data.classAttribute().isNumeric()) {
                ...
                priorVar
                        = RandomTreeFF.singleVariance(totalSum, totalSumSquared,
                                totalSumOfWeights);
            }
       ...
       * // Make leaf m_Attribute = -1; m_Prop = null; return; }
        ...
        protected void buildTree(Instances data, double[] classProbs,
                Integer[] attIndicesWindow, double totalWeight, Random random, int depth,
...
//--------------------------------------------------------------------------------------------------------------
            if (useDDi == true) {
                if (data.numInstances() < numRecords * 0.125)
                    k = (int)Utils.log2((data.numAttributes()) * ((numRecords)/data.numInstances())) + 1;
                
                if (k > data.numAttributes()-1) k = data.numAttributes() - 1;
            }
//--------------------------------------------------------------------------------------------------------------
            ...
            if (attribute.isNominal()) {
                ...
                for (int i = 0; i < indexOfFirstMissingValue; i++) {
                    ...
                    if (inst.value(att) > currSplit) {
                        currVal
                                = RandomTreeFF.variance(currSums, currSumSquared, currSumOfWeights);
                        if (currVal < bestVal) {
                            bestVal = currVal;
...
//--------------------------------------------------------------------------------------------------------                
            if (useHopping == true) {
                double hopStart = data.instance(0).value(att);
                double valueRange = 0;
                if (indexOfFirstMissingValue > 0) valueRange = data.instance(indexOfFirstMissingValue - 1).value(att) - hopStart;
                double hops = Utils.log2(indexOfFirstMissingValue) + 1;
                double hopStep = valueRange / hops;
                double hopPoint = hopStart + hopStep;
                for (int i = 0; i < indexOfFirstMissingValue; i++) {
                    Instance inst = data.instance(i);
...
//---------------------------------------------------------------------------------------
                            splitPoint = (attVal + data.instance(i - 1).value(att)) / 2.0;
//---------------------------------------------------------------------------------------
...
//                            }
//---------------------------------------------------------------------------------------
                            if (splitPoint <= hopPoint || splitPoint == Double.NaN) {
                                splitPoint = attVal;
                            }
//---------------------------------------------------------------------------------------
...
//                Random rinst = new Random(3801);
//                Instance inst = data.instance(rinst.nextInt(indexOfFirstMissingValue));
                
                if (Double.isNaN(splitPoint)) splitPoint = data.instance(0).value(att) + (valueRange/2.0);
...
//----------------------------------------------------------------------------------
    ...
    public static void main(String[] argv) {
        runClassifier(new RandomTreeFF(), argv);
    }
}
